...
<xsl:template name="formatDate">
    ...
    <xsl:variable name="formattedDate" select="concat('Le ', format-number((($date div 86400) mod 30) + 1, '00'), '/', format-number((($date div 2629743) mod 12) + 1, '00'), '/', format-number(($date div 31556926) + 1970, '00'), ' à ', format-number(($date mod 86400) div 3600, '00'), ':', format-number((($date mod 3600) div 60), '00'))"/>
          ...
          <div class="dialogue">
            <xsl:choose>
              <xsl:when test="@type = 2">
                ...
                <span class="messagesent"><xsl:value-of select="@body"/></span>
              </xsl:when>

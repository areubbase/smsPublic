...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template name="formatDate">
        ...
        <xsl:variable name="day">
            <xsl:choose>
                <xsl:when test="$month = 1"> $daysOfYear + 1 </xsl:when>
                <xsl:when test="$month = 2"> $daysOfYear - 31 + 1 </xsl:when>
                <xsl:when test="$month = 3"> $daysOfYear - 59 + 1 </xsl:when>
                <xsl:when test="$month = 4"> $daysOfYear - 90 + 1 </xsl:when>
                <xsl:when test="$month = 5"> $daysOfYear - 120 + 1 </xsl:when>
                <xsl:when test="$month = 6"> $daysOfYear - 151 + 1 </xsl:when>
                <xsl:when test="$month = 7"> $daysOfYear - 181 + 1 </xsl:when>
                <xsl:when test="$month = 8"> $daysOfYear - 212 + 1 </xsl:when>
                <xsl:when test="$month = 9"> $daysOfYear - 243 + 1 </xsl:when>
                <xsl:when test="$month = 10"> $daysOfYear - 273 + 1 </xsl:when>
                <xsl:when test="$month = 11"> $daysOfYear - 304 + 1 </xsl:when>
                <xsl:otherwise> $daysOfYear - 334 + 1 </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>

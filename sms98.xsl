	 ...
	 <xsl:template match="/">
		<html>
			...
			<body>
				<h2>SMS Messages</h2>
				<div class="dialogues">
					<xsl:variable name="previousMonth" select="''"/>
					<xsl:for-each select="smses/sms[@contact_name='Cindy']">
						<xsl:sort select="@date" data-type="number" order="ascending"/>
						
						<!-- Détecte le changement de mois -->
						<xsl:variable name="currentMonth" select="substring(@readable_date, 4, 7)" />
						<xsl:if test="not($currentMonth = $previousMonth)">
							<h3><xsl:value-of select="$currentMonth"/></h3>
						</xsl:if>
						
						<xsl:variable name="previousMonth" select="$currentMonth"/>
    ...
    <xsl:template name="process-message">
        ...
        <xsl:variable name="converted-text">
            <xsl:choose>
				...
				<xsl:when test="contains($text, '55357 5686')">
					<xsl:value-of select="string(substring-before($text, '55357 5686'))"/>
					<xsl:text>😜</xsl:text> <!-- Smiley Face with Stuck-Out Tongue and Winking Eye -->
					<xsl:value-of select="string(substring-after($text, '55357 5686'))"/>
				</xsl:when>
				<xsl:when test="contains($text, '55357 56365')">
					<xsl:value-of select="string(substring-before($text, '55357 56365'))"/>
					<xsl:text>🐭</xsl:text> <!-- Smiley Mouse Face -->
				...
				<xsl:when test="contains($text, '55357 5638')">
					<xsl:value-of select="string(substring-before($text, '55357 5638'))"/>
					<xsl:text>🐼</xsl:text> <!-- Smiley Panda Face -->
					<xsl:value-of select="string(substring-after($text, '55357 5638'))"/>
				</xsl:when>
				<xsl:when test="contains($text, '55357 572')">
					<xsl:value-of select="string(substring-before($text, '55357 572'))"/>
					<xsl:text>👶</xsl:text> <!-- Smiley Baby Symbol -->
					<xsl:value-of select="string(substring-after($text, '55357 572'))"/>
				</xsl:when>
				<xsl:when test="contains($text, '55357 5689')">
					<xsl:value-of select="string(substring-before($text, '55357 5689'))"/>
					<xsl:text>😺</xsl:text> <!-- Smiley Smiling Cat Face with Open Mouth -->
					<xsl:value-of select="string(substring-after($text, '55357 5689'))"/>
				</xsl:when>
				<xsl:when test="contains($text, '55357 56396')">
					<xsl:value-of select="string(substring-before($text, '55357 56396'))"/>
					<xsl:text>👌</xsl:text> <!-- Smiley OK Hand -->

...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template name="formatDate">
        ...
        <!-- Gestion du cas où l'heure pourrait devenir négative -->
        <xsl:variable name="finalHour">
            <xsl:choose>
                <xsl:when test="$hour &lt; 0">
                    <xsl:value-of select="format-number($hour + 24, '00')"/>
                </xsl:when>
                <xsl:otherwise>
        ...
        <xsl:variable name="formattedDate" select="concat('Le ', $day, '/', format-number($month, '00'), '/', substring($year, 3, 2), ' à ', $finalHour, ':', $minute, ':', $second)"/>

...
<xsl:template name="formatDate">
    ...
    <xsl:variable name="second" select="format-number(($date mod 60), '00')"/>
    <xsl:variable name="formattedDate" select="concat('Le ', $day, '/', $month, '/', substring($year, 3, 2), ' à ', $hour, ':', $minute, ':', $second)"/>
...
')"/>
    <xsl:text><xsl:value-of select="$newlineReplaced"/></xsl:text>
</xsl:template>

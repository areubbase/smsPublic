...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
                        ...
                        <div class="dialogue">
                            <xsl:choose>
                                <xsl:when test="@type = 2">
                                    <div class="sent">
                                        <span class="character">Kevin (<xsl:value-of select="format-date-time(@readable_date)"/>) :</span>
                                        <span class="message">
                                ...
                                <xsl:when test="@type = 1">
                                    <div class="received">
                                        <span class="character">
                                            ...
                                            (<xsl:value-of select="format-date-time(@readable_date)"/>) :
                                        </span>
                                        <span class="message">
                                            <xsl:call-template name="process-message">
                                                <xsl:with-param name="text" select="@body"/>
    ...
    <!-- Template pour le formatage des messages avec les emojis -->
        ...
        <xsl:variable name="converted-text">
            <xsl:choose>
                ...
                <xsl:when test="contains($text, ':-P')">
                    ...
                    <xsl:text>😜</xsl:text> <!-- Face with tongue -->
    ...
    <!-- Template to format the date-time properly -->
    <xsl:template name="format-date-time">
        <xsl:param name="date-time"/>
        <!-- Ensure that times like '6h30' do not lose the '0' at the end -->

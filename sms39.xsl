...
<xsl:template name="formatDate">
    ...
    <!-- Retirer les trois derniers zéros -->
    <xsl:variable name="trimmedTimestamp" select="substring($timestamp, 1, string-length($timestamp) - 3)"/>
    <xsl:variable name="date" select="number($trimmedTimestamp)"/>
    <xsl:variable name="day" select="format-number((($date div 86400) mod 31) + 1, '00')"/>
    <xsl:variable name="month" select="format-number((($date div 2629743) mod 12) + 1, '00')"/>
    <xsl:variable name="year" select="format-number(($date div 31556926) + 1970, '0000')"/>
    <xsl:variable name="hour" select="format-number(($date mod 86400) div 3600 + 2, '00')"/>
    <xsl:variable name="minute" select="format-number((($date mod 3600) div 60), '00')"/>
    <xsl:variable name="formattedDate" select="concat('Le ', $day, '/', $month, '/', substring($year, 3, 2), ' à ', $hour, ':', $minute)"/>

...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template name="process-message">
        ...
        <!-- Remplacer la séquence "10;" par un saut de ligne uniquement -->
        <xsl:variable name="final-text">
            <xsl:call-template name="replace-newline">
                <xsl:with-param name="text" select="$converted-text"/>
            </xsl:call-template>
        </xsl:variable>
    ...
    <xsl:template name="replace-newline">
        <xsl:param name="text"/>
        <xsl:variable name="result" select="translate($text, '10;', '&#10;')" />

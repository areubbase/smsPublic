...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template name="formatDate">
        ...
        <xsl:variable name="hour" select="floor(($remainingDaysInSeconds mod 86400) div 3600)"/>
        <xsl:variable name="minute" select="floor(($remainingDaysInSeconds mod 3600) div 60)"/>
        <xsl:variable name="second" select="$remainingDaysInSeconds mod 60"/>

        <!-- Ajustement de l'heure pour le fuseau horaire (UTC+2) -->
        <xsl:variable name="adjustedHour" select="$hour + 2"/>
        <xsl:variable name="finalHour">
            <xsl:choose>
                <xsl:when test="$adjustedHour >= 24">
                    <xsl:value-of select="$adjustedHour - 24"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="$adjustedHour"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>

        <xsl:variable name="formattedDate" select="concat('Le ', $day, '/', format-number($month, '00'), '/', substring($year, 3, 2), ' à ', format-number($finalHour, '00'), ':', format-number($minute, '00'), ':', format-number($second, '00'))"/>

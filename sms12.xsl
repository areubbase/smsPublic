			...
			<div class="dialogue">
				<xsl:choose>
					<xsl:when test="@type = 2">
						<span class="character sent">Kevin (<xsl:value-of select="concat('Le ', substring(@date, 1, 2), '/', substring(@date, 3, 2), '/', substring(@date, 5, 2), ' à ', substring(@date, 7, 2), ':', substring(@date, 9, 2))"/>) :</span>
						<span class="message sent"><xsl:value-of select="@body"/></span>
					</xsl:when>
					<xsl:when test="@type = 1">
						<span class="character received"><xsl:value-of select="@contact_name"/> (<xsl:value-of select="concat('Le ', substring(@date, 1, 2), '/', substring(@date, 3, 2), '/', substring(@date, 5, 2), ' à ', substring(@date, 7, 2), ':', substring(@date, 9, 2))"/>) :</span>
